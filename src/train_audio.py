"""Audio Stage I trainer — FC head on precomputed WavLM features.

Smaller than src/train.py: no encoder fine-tune, no tokenizer, no fp16
needed (model is tiny). Same logger / metrics / early-stop plumbing.

Usage:
    python -m src.train_audio --config configs/iemocap_audio.yaml
"""
...
from .data.audio_dataset import build_audio_loaders
from .models.audio_classifier import build_audio_classifier
...
def evaluate(model, loader, device, label_names) -> Dict:
    ...
    for batch in loader:
        ...
        out = model(features=features, labels=labels)
...
def train(cfg: AttrDict) -> None:
    ...
    model = build_audio_classifier(cfg.model).to(device)
    ...
    if "test" in loaders and best_epoch > 0:
        ...
        ckpt = torch.load(best_ckpt_dir / "audio_stage1.pt", map_location=device, weights_only=True)
